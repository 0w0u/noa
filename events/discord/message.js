module.exports = class event {
  ...
  async run(message) {
    let client = this.client,
      data = {};
    try {
      if (message.guild) data.guild = await client.findOrCreateGuild({ id: message.guild.id });
      message.prefix = message.guild ? data.guild.prefix : client.config.prefix;
      /*     if (client.afk.get(message.author.id)) client.afk.delete(message.author.id), message.channel.send(client.message({ emoji: 'green', razón: 'Â¡bienvenid@ de nuevo!', message })
      if (message.mentions.users.first()) {
        let hijo = client.afk.get(message.mentions.users.first().id);
        if (hijo) message.channel.send(client.message({ emoji: 'red', razón: 'el usuario que has mencionado estÃ¡ AFK por: ' + hijo.reason, message }));
      }*/
      ...
      if (!client.cooldowns.has(cmd.help.name)) {
        client.cooldowns.set(cmd.help.name, new (require('discord.js').Collection)());
      }
      let now = Date.now(),
      ...
      if (data.user.blacklist.bl === true) return message.channel.send(client.message({ emoji: 'red', razón: `¡estás en lista negra del bot! Si crees que esto es un error, dirí­gete al servidor de soporte <https://noabot.xyz/support>`, message }));
      ...
      if (cmd.config.voteOnly && !(await client.dbl.hasVoted(message.author.id))) return message.channel.send(client.message({ emoji: 'red', razón: `necesitas votar por ${client.config.bot} en \`top.gg\` para tener acceso a este comando <https://noabot.xyz/vote/>`, message }));
      if (message.guild) {
        if (message.member) data.member = await client.findOrCreateMember({ id: message.member.user.id, guildID: message.guild.id });
        let a = [];
      ...
      try {
        ...
        let u = new (require('discord.js').MessageEmbed)()
          ...
          .addField('â€¢ Comando', `\`${cmd.help.name}\``)
          .addField('â€¢ Usuario', `\`[${message.author.tag}]\` \`(${message.author.id})\``)
          .addField('â€¢ Servidor', `${message.guild ? `\`[${message.guild.name}]\` \`(${message.guild.id})\`` : '`[Mensajes privados]`'}`);
        client.logs.send(u);
      } catch (e) {
        console.error(e);
      }
