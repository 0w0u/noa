    ...
    try {
      ...
      if (!args[0]) {
        ...
        embed
          ...
          .setDescription(`Creado el **${guild.createdAt.toDateString().split(' ')[2]}/${guild.createdAt.toDateString().split(' ')[1]}/${guild.createdAt.toDateString().split(' ')[3]}** (Hace ${client.functions.checkDays(guild.createdAt)})`)
          .setThumbnail(guild.iconURL())
          .addField('Miembros', `Usuarios: **${guild.members.size}**\nHumanos: **${guild.members.filter(member => !member.user.bot).size}** - Bots: **${guild.members.filter(member => member.user.bot).size}**\n<:online:597588160104366080> \`${guild.members.filter(u => u.presence.status === 'online').size}\`  <:idle:597588160175931442> \`${guild.members.filter(u => u.presence.status === 'idle').size}\`  <:dnd:597588160163348480> \`${guild.members.filter(u => u.presence.status === 'dnd').size}\`\n<:streaming:597588160557613067> \`${guild.members.filter(u => u.presence.game && u.presence.game.streaming).size}\`  <:offline:597588160561807396> \`${guild.members.filter(u => u.presence.status === 'offline').size}\`  📱 \`${guild.members.filter(x => x.presence.clientStatus && x.presence.clientStatus.mobile).size}\``, true)
          .addField('Canales', `Total: **${guild.channels.filter(c => c.type === 'voice').size + guild.channels.filter(c => c.type === 'text').size}**\nCategorías: **${guild.channels.filter(c => c.type === 'category').size}** - Texto: **${guild.channels.filter(c => c.type === 'text').size}** - Voz: **${guild.channels.filter(c => c.type === 'voice').size}**`, true)
