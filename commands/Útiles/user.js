  ...
  async run(message, args, data, embed) {
    ...
    try {
      ...
      async function send(user) {
        let colorEmbed = { online: client.functions.selectColor('green'), idle: client.functions.selectColor('yellow'), dnd: client.functions.selectColor('red'), offline: client.functions.selectColor('lightcolors') },
        ...
        let member = message.guild.member(user),
          days = client.functions.checkDays(user.createdAt),
        ...
        embed
          ...
          .addField('Cuenta', `• Creada el: ${ca[2]}/${ca[1]}/${ca[3]} (Hace ${days})\n• Ingreso al servidor: ${ja[2]}/${ja[1]}/${ja[3]} (Hace ${client.functions.checkDays(member.joinedAt)})\n• Servidores en común: ${client.guilds.filter(g => g.members.has(user.id)).size}`)
        ...
        //.addField('Servidores en común', `${client.guilds.filter(g => g.members.has(info.id)).map(g => `\`${g.name}\``).join(`, `)}`)
        message.channel.send({ embed });
      }
    } catch (e) {
