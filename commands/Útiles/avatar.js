  ...
  async run(message, args, data, embed) {
    ...
    try {
      if (!args[0]) return await send(message.author);
        ...
        if (message.guild.iconURL() === null) return message.channel.send(client.fns.message({ emoji: 'red', razón: 'este servidor no cuenta con un ícono personalizado', message }));
        else {
          embed
        ...
        if (u.length <= 0) return message.channel.send(client.fns.message({ emoji: 'red', razón: 'no hay usuarios que coincidan con tu búsqueda, intenta ser más específico', message }));
        else if (u.length === 1) return await send(u[0].user);
        else if (u.length > 10) return message.channel.send(client.fns.message({ emoji: 'red', razón: 'muchos usuarios coinciden con tu búsqueda, intenta ser más específico', message }));
        else {
