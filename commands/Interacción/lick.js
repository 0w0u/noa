module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'red', razón: 'noargs te veo ansioso, ¡menciona a alguien!', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        ...
        if (user === message.author) return message.channel.send(client.message({ emoji: 'red', razón: 'te diría que suerte lamiéndote, pero guatafo', message }));
        else if (user === client.user) return message.channel.send(client.message({ emoji: 'red', razón: '¡NOOO! *- vomita -*', message }));
        embed
          .setColor(client.fns.selectColor('lightcolors'))
          .setDescription('**' + message.author.username + '** está lamiendo a **' + user.username + '**')
