module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'red', razón: 'menciona con quien debes pelear', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        ...
        if (user === message.author) return message.channel.send(client.message({ emoji: 'red', razón: 'wot, no puedes pelear contigo mismo, loco', message }));
        else if (user === client.user) return message.channel.send(client.message({ emoji: '<:noaMonGun:672934974319493150>', razón: 'no creo que puedas ganarme...', message }));
        embed
          .setColor(client.fns.selectColor('lightcolors'))
          .setDescription('**' + message.author.username + '** está peleando contra **' + user.username + '**')
