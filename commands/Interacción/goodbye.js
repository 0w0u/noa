module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'sad', razón: 'noargs menciona a alguien para despedirte', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        ...
        if (user === message.author) return message.channel.send(client.message({ emoji: 'red', razón: `si solamente quieres despedirte usa \`${message.prefix}bye\``, message }));
        else if (user === client.user) return message.channel.send(client.message({ emoji: 'sad', razón: '¿en serio quieres decirme adiós?', message }));
        embed
          .setColor(client.fns.selectColor('lightcolors'))
          .setDescription('**' + message.author.username + '** se despidió de **' + user.username + '**')
