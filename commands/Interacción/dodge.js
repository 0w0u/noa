module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'noidea', razón: 'noargs menciona a quien quieres esquivar', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        if (user === message.author) return message.channel.send(client.message({ emoji: 'sad', razón: 'no puedes esquivarte a ti mismo', message }));
        else if (user === client.user) return message.channel.send(client.message({ emoji: 'noidea', razón: 'no puedes evitarme jeje...', message }));
        embed
          .setColor(client.fns.selectColor('lightcolors'))
          .setDescription('**' + message.author.username + '** está esquivando a **' + user.username + '**')
