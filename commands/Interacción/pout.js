module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'red', razón: 'noargs menciona a quien quieres hacerle mala cara', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        ...
        if (user === message.author) return message.channel.send(client.message({ emoji: '<:noaThinku:673966812282748968>', razón: '¿crees que puedes hacerte mala cara?', message }));
        else if (user === client.user) return message.channel.send(client.message({ emoji: '<:noaSippu:673964148044070927>', razón: 'a mí no', message }));
        embed
          .setColor(client.fns.selectColor('lightcolors'))
          .setDescription('**' + message.author.username + '** le hizo una mala cara a **' + user.username + '**')
