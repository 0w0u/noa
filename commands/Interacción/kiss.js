module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'red', razón: 'noargs menciona a quien quieres besar', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        if (user === message.author) return message.channel.send(client.message({ emoji: 'red', razón: 'jeje, qué raro', message }));
        else if (user === client.user) return message.channel.send(client.message({ emoji: 'red', razón: 'waaat, no quiero besos', message }));
        embed
