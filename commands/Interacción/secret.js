module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'red', razón: 'noargs ¿a quién le contarás este secreto?', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        ...
        if (user === message.author) return message.channel.send(client.message({ emoji: 'red', razón: 'no te puedes contar un secreto a ti mism@', message }));
        else if (user === client.user) return message.channel.send(client.message({ emoji: 'red', razón: 'no soy esa clase de bots que guardan secretos...', message }));
        embed
          .setColor(client.fns.selectColor('lightcolors'))
          .setDescription('**' + message.author.username + '** le dijo un secreto a **' + user.username + '**')
