module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'red', razón: 'noargs menciona a quien quieres cosquillear', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        ...
        if (user === client.user) return message.channel.send(client.message({ emoji: ':sob:', razón: 'las cosquillas son mi mayor debilidad, ¡no lo hagas!', message }));
        embed
          .setColor(client.fns.selectColor('lightcolors'))
          .setDescription('**' + message.author.username + '** le está haciendo cosquillas a **' + user.username + '**')
