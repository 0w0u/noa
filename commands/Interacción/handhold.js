module.exports = class command extends require('../../base/models/Command.js') {
  constructor(client) {
    super(client, {
      ...
      usage: (prefix) => `\`${prefix + this.help.name} <@usuario>\``,
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let help = this.help;
      if (!args[0]) return message.channel.send(client.message({ emoji: 'red', razón: 'noargs menciona a quien quieres sostener su mano', usage: this.help.usage(message.prefix), message }));
      else {
        if (!isNaN(args[0])) {
      ...
      async function send(user) {
        if (user === message.author) return message.channel.send(client.message({ emoji: 'red', razón: 'no dejaré que sostengas tu propia mano', message }));
        else if (user === client.user) return message.channel.send(client.message({ emoji: 'red', razón: 'no gracias uwu', message }));
        embed
          .setColor(client.fns.selectColor('lightcolors'))
          .setDescription('**' + message.author.username + '** está tomando la mano de **' + user.username + '**')
