module.exports = class command extends require('../../base/models/Command.js') {
  constructor(client) {
    super(client, {
      ...
      usage: (prefix) => `\`${prefix + this.help.name} [@usuario|+imagen+texto]\``,
      examples: (prefix) => `\`${prefix}captcha @mon#0010\``,
      enabled: true,
      cooldown: 5,
      aliases: [],
      botPermissions: [],
  ...
  async run(message, args, data, embed) {
    ...
    try {
      let url, text;
      if (!args[0]) message.channel.send(client.message({ emoji: 'red', razón: 'noargs necesitas agregar un texto para que salga en la imagen', usage: this.help.usage(message.prefix), message }));
      else {
        if (message.attachments.first()) {
          ...
          if (!isNaN(args[0])) {
            ...
            if (message.mentions.users.size > 0) {
              ...
              text = message.mentions.users.first().username;
            } else {
              ...
              if (u.length <= 0) return message.channel.send(client.message({ emoji: 'red', razón: 'no hay usuarios que coincidan con tu búsqueda, intenta ser más específico', message }));
                ...
                url = u[0].user.displayAvatarURL({ format: 'png', size: 2048 });
                ...
                if (!i) {
                  ...
                  text = u[i.content - 1].displayName;
                  msg.delete({ timeout: 5000 });
                }
              }
            }
      ...
      let msg = await message.channel.send(client.fns.reply('generating', message)),
        img = await require('node-superfetch').get(`https://eclyssia-api.tk/api/v1/captcha?url=${url}&username=${text}`);
      msg.delete();
