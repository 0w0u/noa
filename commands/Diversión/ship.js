module.exports = class command extends require('../../base/models/Command.js') {
  ...
  async run(message, args, data, embed) {
    ...
    try {
      if (message.mentions.members.size < 1 || !args[0]) message.channel.send(client.message({ emoji: 'red', razón: 'debes de mencionar a 2 personas para shippear', message }));
      else {
        ...
        message.channel.send(client.message({ emoji: 'heart', razón: `**${shipped.displayName}** y **${shipper.displayName}**\nEl shippeo ideal es: **${final_name}**`, usage: this.help.usage(message.prefix), message }));
      }
    } catch (e) {
      client.err({
        type: 'command',
