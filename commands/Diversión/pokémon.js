  ...
  async run(message, args, data, embed) {
    ...
    try {
      ...
      if (!attempts) {
        ...
        message.channel.send(client.message({ emoji: 'red', razón: 'tardaste mucho en escribir, ¡al pokémon le dio tiempo a huir!', usage: this.help.usage(message.prefix), message }));
        return;
      }
      attempts = attempts.content.toLowerCase();
      if (attempts === pokem.name.toLowerCase()) {
        ...
        message.channel.send(client.message({ emoji: 'red', razón: 'oops, te equivocaste y el pokémon escapó', usage: this.help.usage(message.prefix), message }));
        return;
      }
    } catch (e) {
      client.err({
